...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  return (
    <>
      ...
      <div className={`gooey-nav relative ${className}`} ref={containerRef}>
        <nav className="flex relative" style={{ transform: 'translate3d(0,0,0.01px)' }}>
          <ul
            ref={navRef}
            className="relative z-[3] m-0 flex list-none gap-3 p-0"
            style={{
              color: '#eaf2ff',
          ...
          >
            {items.map((item, index) => (
              <li
                key={item.href}
                className={`relative cursor-pointer rounded-full text-ice transition-[color_text-shadow] duration-300 ease ${
